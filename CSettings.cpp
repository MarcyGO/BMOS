...
int CSettings::GetAudioSetting(int numid)
{
    ...
    if (numid == 1)
    {
        sprintf(volCmd, "/home/bmo/bmos/scripts/getmic.sh");
    }
    else if (numid == 2)
    {
        sprintf(volCmd, "/home/bmo/bmos/scripts/getvolume.sh");
    }
    else
    {
        return(-1);
...
int CSettings::SetAudioSetting(int numid, int val)
{
    ...
    sprintf(volCmd, "/home/bmo/bmos/scripts/vol.sh %d %d", val, numid);
#ifndef WINDOWS
    system(volCmd);
#endif
